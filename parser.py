...
def main():
    ...
    for idx, path in enumerate(paths):
        ...
        data = {'temp0': temp0, 'temp1': temp1, 'time': time}
        df = pd.DataFrame(data)

        sns.set_theme(style="darkgrid")
        sns.scatterplot(x='time', y='temp0', data=df)
        sns.scatterplot(x='time', y='temp1', data=df)
        plt.errorbar(df.time, df.temp0, yerr=0.1 * df.temp0, alpha=0.2)
        plt.errorbar(df.time, df.temp1, yerr=0.1 * df.temp1, alpha=0.2)
        plt.legend(labels=["temp0", "temp1"])
        plt.xlabel('time [s]')
